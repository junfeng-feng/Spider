...
#                 "http://www.to8to.com/ask/k866853.html",
    ...
    def parse(self, response):
        ...
        item["image_urls"] = []
        try:
            item["image_urls"] = select.css(".ask_qustion")[0].xpath(".//img/@src").extract()
        except Exception, e:
            print e
        ...
        try:        
            ...
            try:
                # best
                item["answer_id"] = question_id + "-" + "best"
                item["answer_content"] = select.css(".best_answer").xpath("./p").extract()[0][21:-4]
                item["is_best"] = "yes"
                # 如果最佳答案有图片
                try:
                    item["image_urls"] = ["http://www.to8to.com"+src for src in select.css(".best_answer").xpath("./img/@src").extract()]
                except Exception, e:
                    print e
            ...
            for li in ask_answer_li_list:
                ...
                item["answer_id"] = question_id + "-" + pageNo + "-" + str(index)
                ...
                item["image_urls"] = []
                try:
                    item["image_urls"] = ["http://www.to8to.com" + src for src in li.xpath("./img/@src").extract()]
                except Exception, e:
                    print e
