...
class tamllproduct_spider(Spider):
    ...
    '''
    for brandId in file(r"D:\coding\workspace\Projects\tmall\tmall\spiders\brandid.list"):
        brandId = brandId.strip()
        start_urls.append("https://list.tmall.com//search_product.htm?brand=%s" % (brandId))
        break
    '''
    for line in file(r"D:\coding\workspace\Projects\tmall\tmall\spiders\tmall_brand.txt"):
        content = line.split(';')
        brandId = content[0]
        is_decoration = content[-1]
        if is_decoration.strip() == 'yes':
            start_urls.append("https://list.tmall.com//search_product.htm?brand=%s" % (brandId))
            brandid_isdecoration[brandId] = is_decoration.strip()
    ...
    def parse(self, response):
        ...
        if len(itemList) > 1:#当前有产品，才需要处理
            for productItem in itemList:
                ...
                #是不是装修类
                item["is_decoration"] = self.brandid_isdecoration[brandId]
